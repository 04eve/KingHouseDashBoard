...
function Category() {
    const columns = [
        ...
        { Header: "categoryName", accessor: "categoryName", align: "left" },
    ...
    const deleteCategory = async (id) => {
        if (window.confirm('Are you sure you want to delete this category?')) {
            ...
            const remainedRows = rows.filter((categoryName) => {
    ...
    useEffect(() => {
        const jsxRows = rows?.map((category) => {
            return {
                ...
                image: <> <img src={category.image} alt="Girl in a jacket" width="150" height="160"/></>,
                options: <>
